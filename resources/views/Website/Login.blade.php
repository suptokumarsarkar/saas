...
@push('headerScript')
    ...
    <link rel="stylesheet" href="{{asset('public/css/login_style.css' . App\Logic\Helpers::production("version"))}}">
    <link rel="stylesheet"
          href="{{asset('public/css/login_responsive.css' . App\Logic\Helpers::production("version"))}}">
@endpush
@section("content")
    {!! App\Logic\Notification::get('top')  !!}

    {{--Include Header--}}
    @includeIf('Website.Components.Common.LoginHeader')

    {{--Include Body--}}
    @if(isset($redirectTo))
        @includeIf('Website.Components.Login.Body')
    @else
        @includeIf('Website.Components.Login.Body')
    @endif
