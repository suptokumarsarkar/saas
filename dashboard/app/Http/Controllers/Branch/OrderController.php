...
class OrderController extends Controller
{
    ...
    public function tablebookingStatus(Request $request){
        if($request->id){
            if($order = Tablebooking::find($request->id)){
        ...
        Toastr::warning('Failed To Get Order');
        return back();
    }
    ...
    public function tablebooking()
    {
        ...
        Tablebooking::where([ 'checked' => 0,'branchId' => auth('branch')->id()])->update(['checked' => 1]);
        $orders =Tablebooking::where([['branchId' , auth('branch')->id()],['created_at', '>', time()-30*24*3600]])->get();

        $query_param = [];
        foreach($orders as $order)
        {
            if(User::find($order->customerId)){
                $order->customer = User::find($order->customerId);
            }
        }

        return view('branch-views.order.tableBooking', compact('orders'));
    }
