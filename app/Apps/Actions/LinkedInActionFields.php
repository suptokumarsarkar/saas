...
class LinkedInActionFields
{
    ...
    public function create_share_update($data)
    {
        // Labels
    ...
    public function create_share_update_post($accountId, $mainData, $data)
    {
        $this->account = Account::find($accountId);
        $this->mainClass = new LinkedIn;

        $this->access_token = $this->mainClass->getToken($accountId);
        $user_id = $this->mainClass->getUserId($accountId);
        if (isset($mainData['comment'])) {
            $params = [
                'user_id' => $user_id,
                'comment' => Helpers::stringorexplode($mainData['comment']),
                'desc' => Helpers::stringorexplode($mainData['desc']),
                'image_url' => Helpers::stringorsingle($mainData['image_url'] ?? []),
                'content_url' => Helpers::stringorsingle($mainData['content_url'] ?? []),
            ];
            $this->mainClass->shareContent($this->access_token, $params);
